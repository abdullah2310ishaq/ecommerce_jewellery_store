...
export default function SearchWithModal() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      try {
        ...
        const qSearch = query(ref, where("name", ">=", searchQuery), where("name", "<=", searchQuery + "\uf8ff"))
  ...
  }, []) // Removed closeDropdown from dependencies
    ...
    <div ref={containerRef} className="relative w-full max-w-md">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full px-4 py-2 pl-10 text-gray-600 bg-gray-50 border border-gray-200 rounded-full focus:outline-none focus:ring-2 focus:ring-[#FB6F90] focus:border-transparent text-sm"
        />
      </div>

      {/* Dropdown panel with refined animation */}
      <AnimatePresence>
        {open && (
          <motion.div
            className="absolute left-0 right-0 mt-2 bg-white border border-gray-100 rounded-lg shadow-lg z-40"
            initial={{ opacity: 0, y: -10 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
          >
            {/* Top bar with elegant styling */}
            <div className="flex justify-between items-center p-3 border-b border-gray-50">
              <h3 className="text-sm font-medium text-gray-700">Search Results</h3>
              <button onClick={closeDropdown} className="text-gray-400 hover:text-[#FB6F90]">
                <X className="w-5 h-5" />
              </button>
            </div>

            {selectedProduct ? (
              // --- PRODUCT DETAIL VIEW with refined styling ---
              <div className="p-4">
                <button
                ...
                <p className="text-lg text-[#FB6F90] my-2">Rs. {selectedProduct.price}</p>
              ...
              // --- SEARCH INPUT & RESULTS with refined styling ---
              <>
                <div className="px-4 py-3">
                  <div className="relative mb-3">
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2 pl-10 text-gray-700 bg-gray-50 border border-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-[#FB6F90] text-sm"
                      autoFocus
                    />
                  </div>
                    ...
                    {loading ? (
                      ...
                      results.map((product) => (
                        <div
                          ...
                          className="flex items-center p-3 border-b border-gray-50 last:border-none cursor-pointer hover:bg-gray-50 transition-colors"
                        >
                          ...
                          <div>
                            ...
                            <p className="text-xs text-[#FB6F90] font-medium">Rs. {product.price}</p>
                          </div>
                        </div>
                      ))
                    ) : searchQuery.trim() ? (
                      <p className="p-4 text-center text-gray-500 text-sm">No products found</p>
                    ) : (
                      <p className="p-4 text-center text-gray-500 text-sm">Start typing to search...</p>
                    )}
                  </div>
                </div>
              </>
